...
from std_msgs.msg import Bool, Float32MultiArray, Float64, Int32, Int32MultiArray
...
class Recorder(Node):
    def __init__(self, check: bool, rgb: bool = False,
        ...
        self.rgb_t = []                           # 수신 시각 (perf_counter, 19_real_time_global 과 동일 시계)
    ...
    def _tick(self):
        ...
        if self.rgb:
            row["41_rgb_index"] = np.array(self._rgb_index_for_tick(), dtype=np.int32)
    ...
    def _open_file(self):
        ...
        # 시계 앵커: 19_real_time_global 은 perf_counter(프로세스 기준 단조시계)라 이 값만으로는
    ...
    def _save_demo(self):
        ...
        rgb_note = ""
        if self.rgb:
            # 40_rgb_jpeg: 고유 JPEG 프레임(가변길이). 이미 압축된 데이터라 gzip 은 걸지 않는다.
            # 41_rgb_index: 스텝(N)별로 40 의 어느 프레임을 보고 있는지 (-1 = 아직 없음)
            # 42_rgb_time : 수신 시각(perf_counter) — 19_real_time_global 과 같은 시계라 바로 정렬 가능
            # 43_rgb_stamp: 카메라 캡처 시각(ROS header) — 전송 지연 확인용
            vlen = h5py.vlen_dtype(np.uint8)
            ds = grp.create_dataset("40_rgb_jpeg", (len(self.rgb_frames),), dtype=vlen)
            for i, b in enumerate(self.rgb_frames):
                ds[i] = np.frombuffer(b, dtype=np.uint8)
            grp.create_dataset("42_rgb_time", data=np.asarray(self.rgb_t, dtype=np.float64))
            grp.create_dataset("43_rgb_stamp", data=np.asarray(self.rgb_stamp, dtype=np.float64))
            mb = sum(len(b) for b in self.rgb_frames) / 1e6
            grp.attrs["n_rgb_frames"] = len(self.rgb_frames)
            rgb_note = f", RGB {len(self.rgb_frames)}장 {mb:.1f}MB"
        self.f.flush()
        self.get_logger().info(
    ...
    def _check_report(self):
        ...
        if self.rgb:
            ...
            info = (f"{m.get('width','?')}x{m.get('height','?')} {m.get('format','?')}"
                    if m else "메타 없음")
            print(f"  {ok:14s} {'40_rgb_jpeg':28s} <- {RGB_TOPIC}")
...
def main():
    ...
    ap.add_argument("--rgb-on", action="store_true",
                    help="RGB(JPEG) 도 저장. 카메라가 이미 JPEG 이라 재인코딩 없이 그대로 담는다")
