...
발행 (record/ros2_hdf5_recorder.py 의 20/21번 필드와 동일):
    /paxini/<side>/ft    Float32MultiArray[12]    4손가락 × (Fx,Fy,Fz)
    /paxini/<side>/raw   Float32MultiArray[1524]  4손가락 × 127점 × (x,y,z)
...
class PaxiniUartNode(Node):
    def __init__(self, side: str, port: str, calibrate: bool, print_hz: float):
        ...
        self.pub_ft = self.create_publisher(
            Float32MultiArray, f"/paxini/{side}/ft", SENSOR_QOS)
        self.pub_raw = self.create_publisher(
            Float32MultiArray, f"/paxini/{side}/raw", SENSOR_QOS)
        ...
        while not self._stop.is_set():
            ...
            try:
                ...
                self.get_logger().info(
                    ...
                    f"/paxini/{self.side}/ft, /paxini/{self.side}/raw")
...
def main():
    ...
    rclpy.init()
    node = PaxiniUartNode(a.side, a.port, a.calibrate, a.print_hz)
    try:
        rclpy.spin(node)
