...
def tactile_reader(sh: Shared, port: str, calibrate: bool):
    ...
    while not sh.stop.is_set():
        ...
        try:
            ...
            # 이전 실행이 auto-push 를 켠 채로 끝나면 데이터 프레임이 계속
            # 흘러든다. enable 응답 헤더와 데이터 프레임 헤더가 둘 다 AA56 이라
            # 버퍼를 비우지 않으면 응답 파서가 데이터 프레임을 응답으로 읽고
            # LRC 검사에 실패한다. paxini_uart_node._open() 과 같은 순서.
            ...
            if calibrate:
                ...
                time.sleep(0.5)
                ser.reset_input_buffer()            # 보정 응답(AA55) 비움
            enable_ok = start_auto_push_stream(ser)
            with sh.lock:
                # enable 응답은 오판되기 쉬워 실패로 죽이지 않는다. 실제 스트림
                # 도착 여부는 아래 수신 루프가 판정한다.
                sh.tac_err = None if enable_ok else "enable 응답 불확실 — 스트림 대기"
        except Exception as e:                                  # noqa: BLE001
            with sh.lock:
                sh.tac_err = str(e)
            try:
